...
module StormDiagramComponent =
    ...
    let randomHsl () =
        "hsla(" + string (Random().Next 360) + ", 85%, 25%, 1)"
        ...
        | _ :: _ ->
            ...
            let sources =
                ...
                |> List.mapi (fun i (name, color) ->
                    ...
                    node.setPosition 10 (10 + (i * 40) + (i * (Bindings.layers.Length * 17)))
                    ...
                    let ports =
                        ...
                        |> List.map (fun (layer, warm) ->
                            let port =
                                node.addOutPort
                                    (layer + (if warm = "" then "" else (sprintf "\u00A0\u00A0->\u00A0\u00A0(%s)" warm)))
            ...
            let featureStore =
                ...
                |> List.map (fun (name, color) ->
                    let features =
                        bindings
                        |> Seq.filter (fun (_, (fullFeature, _)) -> fullFeature.StartsWith (name + string Bindings.separator))
                        |> Seq.map snd
                        |> Seq.distinct
