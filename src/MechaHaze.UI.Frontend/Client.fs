...
module Client =
    let inline handleClientMessage (message: SharedState.SharedServerMessage) (state: UIState.State) =
        ...
        | SharedState.SetTrack track ->
            let state =
                { state with SharedState = { state.SharedState with Track = track } }
            state, None
            
        | SharedState.ClientSetDebug debug ->
            let state =
                { state with SharedState = { state.SharedState with Debug = debug } }
            state, None
        ...
        | SharedState.ClientSetAutoLock autoLock ->
            let state =
                { state with SharedState = { state.SharedState with AutoLock = autoLock } }
            state, Some (SharedState.SetAutoLock autoLock)
            
        | SharedState.ClientSetRecordingMode recordingMode ->
            let state =
                { state with SharedState = { state.SharedState with RecordingMode = recordingMode } }
        ...
        | SharedState.ClientToggleBinding binding ->
            state, Some (SharedState.ToggleBinding (Bindings.BindingToggle (state.SharedState.ActiveBindingsPreset, binding)))
            
        | SharedState.ClientTogglePreset presetName ->
            state, Some (SharedState.TogglePreset presetName)
        
        | SharedState.ClientSetActiveBindingsPreset presetName ->
            let state =
                { state with SharedState = { state.SharedState with ActiveBindingsPreset = presetName } }
