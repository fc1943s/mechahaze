    ...
    let ``default`` = FunctionComponent.Of (fun props ->
        let state =
            Hooks.useReducer ((fun state dispatch ->
                ...
                | ToggleBindingSource (bindingSource, layer) ->
                    ...
                    { state with SelectedPeaks = state.SelectedPeaks |> Set.toggle id }
            ), State.Default)
        
        let events = {|
            OnMenuBindingSourcePeakClick = fun bindingSource layer _ ->
                state.update (ToggleBindingSource (bindingSource, layer))
          
            OnMenuDebugClick = fun _ ->
                props.Dispatch (SharedState.ClientSetDebug (not props.UIState.SharedState.Debug))

            OnBindingsDestinationToggle = fun id ->
                props.Dispatch (SharedState.ClientToggleBinding (Bindings.Binding ("", id)))
                
            OnPresetToggle = fun presetName ->
                props.Dispatch (SharedState.ClientTogglePreset presetName)
                
            OnActiveBindingsPresetChange = fun presetName ->
                props.Dispatch (SharedState.ClientSetActiveBindingsPreset presetName)

            OnMenuTopRightButtonClick = fun _ ->
                printfn "CLICK"

            OnOffsetSliderChange = fun (e: Event) ->
                props.Dispatch (SharedState.ClientSetOffset (float e.Value))
            ...
            OnAlignmentAutoLockClick = fun _ ->
                props.Dispatch (SharedState.ClientSetAutoLock (not props.UIState.SharedState.AutoLock))

            OnAlignmentRecordingModeClick = fun _ ->
                props.Dispatch (SharedState.ClientSetRecordingMode (not props.UIState.SharedState.RecordingMode))

            OnAlignmentOffsetCenterClick = fun _ ->
                props.Dispatch (SharedState.ClientSetOffset 0.)

            OnAlignmentOffsetFlipClick = fun _ ->
                props.Dispatch (SharedState.ClientSetOffset -props.UIState.SharedState.Track.Offset)

            OnAlignmentLockClick = fun _ ->
                props.Dispatch (SharedState.ClientSetLocked (not props.UIState.SharedState.Track.Locked))
        |}
        ...
        Text.div [ Props [ Style [ Height "100%" ] ]
                ...
                Navbar.Item.div [ Navbar.Item.HasDropdown
                    ...
                    Navbar.Dropdown.div [][
                        ...
                        Navbar.Item.div [][
                            ...
                            div [ ClassName "field"
                                ...
                                Checkbox.input [ CustomClass "switch is-small is-dark"
                                                 Props [ Checked props.UIState.SharedState.Debug
                                                         OnChange (fun _ -> ()) ]]
                        ...
                        |> List.map (fun bindingSource ->
                            div [ Key bindingSource
                                  Class "navbar-nested-dropdown" ][
                                
                                Navbar.Item.div [][
                                    str (bindingSource.[0].ToString().ToUpper () + bindingSource.Substring 1)
                                    Icon.icon [ Icon.Props [ Style [ Float FloatOptions.Right ] ] ][
                                ...
                                Dropdown.menu [][
                                    Navbar.Dropdown.div [] [
                                        ...
                                        |> List.map (fun (layer, _) ->
                                            ...
                                            Navbar.Item.div [ Navbar.Item.Props [ Key layer
                                                                                  OnClick (events.OnMenuBindingSourcePeakClick
                                                                                               bindingSource
                                                                                               layer) ]][

                                                Checkbox.input [ CustomClass "switch is-small is-dark"
                                                                 Props [ Checked (state.current.SelectedPeaks |> Set.contains id)
                                                                         OnChange (fun _ -> ()) ]]
            ...
            if props.UIState.SharedState.Track.Id <> "" then
                PeaksComponent.``default`` { Track = props.UIState.SharedState.Track
            ...
            div [ Style [ Position PositionOptions.Relative ] ][
                ...
                Slider.slider [ Slider.IsFullWidth
                                Slider.Color IsDark
                                Slider.Props [ Style [ Margin "5px 0" ] ]
                                Slider.Min -0.5
                                Slider.Max 0.5
            ...
            Columns.columns [ Columns.Props [ Style [ Margin -5
                                                      CSSProp.Overflow OverflowOptions.Auto ] ] ][

                Column.column [ Column.Width (Screen.All, Column.Is4)
                                Column.Props [ Style [ MinWidth 855
                                                       Position PositionOptions.Relative ] ] ][

                    (* Tab 1.1 *)
                    Tabs.tabs [ Tabs.Size IsSmall
                    ...
                    div [ Style [ Position PositionOptions.Absolute
                        ...
                        Dropdown.dropdown [ Dropdown.IsHoverable ][
                            ...
                            Dropdown.menu [][
                                Dropdown.content [ Props [ Style [ Padding "15px 20px" ] ] ][
                                    
                                    BindingsSettingsDropdownComponent.``default``
                                        { BindingsPresetMap = props.UIState.SharedState.BindingsPresetMap
                                          ActiveBindingsPreset = props.UIState.SharedState.ActiveBindingsPreset
                    ...
                    div [ Class "storm-diagram-container"
                ...
                Column.column [][
                    ...
                    let data =
                        ...
                        |> Seq.sortByDescending (fun { Offsets = offsets } -> offsets |> Seq.last |> SharedState.ofTimeSyncOffset |> fst)
                        |> Seq.tryHead
                        |> function
                            | None -> [||]
                            | Some timeSync ->
                ...
                Column.column [ Column.Width (Screen.All, Column.Is2) ][
                    ...
                    Columns.columns [][
                        ...
                        Column.column [][
                            str "Offset: "; ofFloat props.UIState.SharedState.Track.Offset
                        ]

                        Column.column [ Column.Props [ Style [ PaddingTop 17 ] ]][

                            div [ ClassName "field"
                                  OnClick events.OnAlignmentAutoLockClick ][

                                Checkbox.input [ CustomClass "switch is-small is-dark"
                                                 Props [ Checked props.UIState.SharedState.AutoLock
                                                         OnChange (fun _ -> ()) ]]
                            ...
                            div [ ClassName "field"
                                ...
                                Checkbox.input [ CustomClass "switch is-small is-dark"
                                                 Props [ Checked props.UIState.SharedState.RecordingMode
                                                         OnChange (fun _ -> ()) ]]
                    ...
                    Columns.columns [][
                        ...
                        Column.column [][
                            ...
                            div [ ClassName "field"
                                ...
                                Checkbox.input [ CustomClass "switch is-small is-dark"
                                                 Props [ Checked props.UIState.SharedState.Track.Locked
                                                         OnChange (fun _ -> ()) ]]
                    ...
                    | track ->
                        str "- Id: "; str track.Id; br []
                        str "- Position: "; ofFloat track.Position
                    ...
                    if props.UIState.SharedState.Debug then
                        ...
                        str "User Id: "; str props.PrivateState.User.Id; br []
                        str "Connection: "; str (sprintf "%A" props.PrivateState.Connection); br []
