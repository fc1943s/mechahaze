...
module ExtTypes =
    ...
    type IStormInteractive =
        ...
        member this.AddEntityRemovedEvent (x: obj -> unit) =
            this.registerListener
                {|
                    entityRemoved =
                        (fun a b c ->
                            printfn "B %A %A %A" a b c
                            x a)
                |}
    ...
    type IStormLinkModel =
        ...
        member this.AddSourcePortChangedEvent (x: obj -> unit) =
            this.registerListener
                {|
                    sourcePortChanged =
                        (fun a b c ->
                            printfn "C %A %A %A" a b c
                            x a)
                |}

        member this.AddTargetPortChangedEvent (x: obj -> unit) =
            this.registerListener
                {|
                    targetPortChanged =
                        (fun a b c ->
                            printfn "D %A %A %A" a b c
                            x a)
                |}
    ...
    type IStormDiagramModel =
        ...
        member this.AddNodesUpdatedEvent (x: obj -> unit) =
            this.registerListener
                {|
                    nodesUpdated =
                        (fun a b c ->
                            printfn "E %A %A %A" a b c
                            x a)
                |}

        member this.AddLinksUpdatedEvent (x: obj -> unit) = this.registerListener {| linksUpdated = x |}

        member this.AddOffsetUpdatedEvent (x: obj -> unit) =
            this.registerListener
                {|
                    offsetUpdated =
                        (fun a b c ->
                            printfn "G %A %A %A" a b c
                            x a)
                |}

        member this.AddZoomUpdatedEvent (x: obj -> unit) =
            this.registerListener
                {|
                    zoomUpdated =
                        (fun a b c ->
                            printfn "H %A %A %A" a b c
                            x a)
                |}

        member this.AddGridUpdatedEvent (x: obj -> unit) =
            this.registerListener
                {|
                    gridUpdated =
                        (fun a b c ->
                            printfn "I %A %A %A" a b c
                            x a)
                |}
    ...
    type IStormDiagrams =
        ...
        member this.CreateDiagramModel () = JsInterop.createNew this.DiagramModel () :?> IStormDiagramModel
