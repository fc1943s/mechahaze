...
module ModalManager =
    ...
    type State =
        ...
        static member inline Default = { Visible = false; ConfirmCallback = fun _ -> () }
        ...
        let state =
            Hooks.useReducer
                ((fun state dispatch ->
                    ...
                    match dispatch with
                    | Toggle -> { State.Default with Visible = not state.Visible }

                    | ConfirmRequest callback -> { Visible = true; ConfirmCallback = callback }

                    | Confirm ->
                        JS.setTimeout (fun _ -> state.ConfirmCallback ()) 0
                        |> ignore
