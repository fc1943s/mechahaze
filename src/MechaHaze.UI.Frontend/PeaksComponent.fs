...
module PeaksComponent =
    ...
    let view = FunctionComponent.Of (fun (__initialProps: Props) ->
        let state =
            Hooks.useState
                { PeaksHandle = null
                  LastPosition = 0.
    ...
    type PeaksComponent (initialProps) =
        ...
        member this.GetTrackDataUri () =
            ...
            then JS.undefined
                        ...
                        JS.encodeURIComponent
                            (sprintf "root/db-tracks/%s/%s.%s.peaks.%s.dat"
                                 this.props.Track.Id
                                 this.props.Track.Id
                                 this.props.Layer
        ...
        member this.Init () =
            if this.state.PeaksHandle = null && this.props.Track.Id <> "" then
                let peaksOptions =
                    {| containers = {| overview = document.getElementById ids.overviewContainer
                       ...
                       zoomLevels = [| 256 |] |> Util.jsArray
                       segments = [| {| startTime = 30
                                        endTime = 60
                                        color = "#333" |} |]
                       points = [| {| time = 5
                                      color = "#BBB" |} |]
                ...
                Ext.peaks.init peaksOptions (System.Action<_, _> onPeaksReady) |> ignore
        ...
        override this.render () =
            div [] [
                ...
                div [ Id ids.waveformContainer
                      Style [ Position PositionOptions.Relative
                              PointerEvents (if (this.props.Debug) then "auto" else "none") ] ] [
                    ...
                    audio [ Id ids.audio
                        ...
                        source [ HTMLAttr.Type "audio/mpeg"
                                 Src (this.GetTrackMediaUrl ()) ]
                    ]
                    
                    div [ Id ids.zoomviewContainer ] []
                    div [ Id ids.overviewContainer ] []
                ]
            ]
        ...
        override this.componentDidUpdate (__prevProps, ___prevState) =
            ...
            if this.state.PeaksHandle <> null then
                ...
                if this.props.Track.Position > 0. then
                    if this.state.LastPosition <> this.props.Track.Position then
                        if this.state.LastId = "" || this.state.LastId <> this.props.Track.Id then
                            let props =
                                {| dataUri = this.GetTrackDataUri ()
                        ...
                        let diff = (float (DateTime.UtcNow.Ticks - this.props.Track.Timestamp) / 10000. / 1000.) - this.props.Track.Offset
                        printfn "diff %A" diff
    ...
    let ``default`` props =
        ofType<PeaksComponent, _, _> props []

    let default2 props =
        reactiveCom
            (fun __props -> 0)
            (fun __message __state -> 0)
            (fun __model __dispatch -> str "")
            ""
            props []
