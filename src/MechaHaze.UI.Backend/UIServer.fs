...
module UIServer =
    ...
    type UIServer () =
        ...
        member this.BroadcastTrack track =
            None |> Option.iter (fun _ ->
                Log.Verbose ("Sending Track to clients. Connections: {@Connections}. State: {Track}", connections.GetConnectedUsers(), track)
            )
            connections.SharedBroadcastToClients (SharedState.SetTrack track)
            
        member this.BroadcastState state =
            None |> Option.iter (fun _ ->
                Log.Verbose ("Sending state to clients. Connections: {@Connections}. State: {Track}", connections.GetConnectedUsers(), state)
            )
            connections.InternalBroadcastToClients (InternalUI.InternalServerMessage.SetState state)
            
            
        member this.HangAsync stateQueue = async {
            
            let handleClientMessage message (state: UIState.State) __serverToClientDispatch =
                match message with
                | SharedState.SetOffset offset ->
                    let state =
                        { state with SharedState = { state.SharedState with Track = { state.SharedState.Track with Offset = offset } } }
                    state, None
                    
                | SharedState.SetAutoLock autoLock ->
                    { state with SharedState = { state.SharedState with AutoLock = autoLock } }, None
                    
                | SharedState.SetRecordingMode recordingMode ->
                    { state with SharedState = { state.SharedState with RecordingMode = recordingMode } }, None
                    
                | SharedState.SetLocked locked ->
                    let state =
                        { state with
                              SharedState = { state.SharedState with
                                                  Track = { state.SharedState.Track with
                                                                Locked = locked
                                                                Offset = if not locked then 0. else state.SharedState.Track.Offset } } }
                    state, None
                ...
                | SharedState.ToggleBinding (Bindings.BindingToggle (presetName, binding)) ->
                    let state =
                        ...
                        { state with SharedState = { state.SharedState with BindingsPresetMap = presets } }
                            
                    state, None
                    
                | SharedState.TogglePreset presetName ->
                    let state =
                        let presetMap = state.SharedState.BindingsPresetMap |> Bindings.ofPresetMap
                        ...
                        let presets =
                            ...
                            |> function
                                ...
                                | false -> presetMap |> Map.add presetName (Bindings.Preset [])
                            |> Bindings.PresetMap
                ...
                | SharedState.SetActiveBindingsPreset presetName ->
                    { state with SharedState = { state.SharedState with ActiveBindingsPreset = presetName } }, None
