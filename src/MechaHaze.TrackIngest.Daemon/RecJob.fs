...
module RecJob =
    let private persistFingerprintAsync (path: string) id = async {
        ...
        if trackData = null then
            try
                let! hashedFingerprints = FingerprintCommandBuilder
                                              .Instance
            ...
            with ex ->
                exn ("Error inserting fingerprints", ex) |> raise
        else
            Log.Debug ("Track ID already found on fingerprint database")
    }

    let persistWaveformAsync sourcePath destPath = async {
        Log.Debug ("Persisting waveform")

        let! exitCode, _ =
            Runtime.executePowerShellAsync 
                [ sprintf """ %s -i "%s" -o "%s" -b 8 --split-channels """
    ...
    let wavTo16 path =
        ...
        do
            use reader = new AudioFileReader (path32)
            WaveFileWriter.CreateWaveFile16 (path, reader)

        File.Delete path32

    let waveFormat16 (waveFormat: WaveFormat) =
        WaveFormat (waveFormat.SampleRate, 16, waveFormat.Channels)

    let recombineAudio16 slices (combinedPath: string) =

        let buffer =
            slices
            |> Seq.map (fun path -> seq {
    ...
    let ensureSameWavLengthAsync source destination = async {
        ...
        if diff < 0L then
            ...
            do
                ...
                sourceReader.Read (buffer, 0, buffer.Length) |> ignore
                destinationFillerWriter.Write (buffer, 0, buffer.Length)

            recombineAudio16 [| destinationBad; destinationFiller |] destination
    ...
    let private ingestFileAsync (path: string) = async {
        Log.Debug ("\n\nIngesting file: {FileName}", Path.GetFileName path)

        let id =
            let id = Path.GetFileNameWithoutExtension path

            if Regexxer.hasMatch (id, "\[id=(\d+)\]")
            then id
            else id + (sprintf " [id=%s]" (DateTime.Now.ToString "yyyyMMddHHmmssfff"))
            ...
            try
                ...
                let splitWav path =
                    ...
                    |> Array.Parallel.map (fun i ->
                        let startPos, endPos =
                            let getOffset i = int64 (i * int sliceSeconds * (reader.WaveFormat.SampleRate * reader.WaveFormat.BlockAlign))

                            getOffset i, min reader.Length (getOffset (i + 1))

                        let newPath = Path.Combine (Path.GetDirectoryName path, sprintf "%d.%s" i (Path.GetFileName path))
                ...
                do!
                    Bindings.layers
                    |> Seq.map (fun (layer, warm) -> async {
                        let tempOutDir = Path.Combine (tempFolder, layer)
                        Directory.CreateDirectory tempOutDir |> ignore

                        try
                            let destPeaksLevelsPath = Path.Combine (trackPath, sprintf "%s.%s.peaks.%s.dat" id layer Bindings.sources.Levels)
                            let destPeaksPitchPath = Path.Combine (trackPath, sprintf "%s.%s.peaks.%s.dat" id layer Bindings.sources.Pitch)
                            let destPitchPath = Path.Combine (trackPath, sprintf "%s.%s.%s.csv" id layer Bindings.sources.Pitch)

                            let destPeaksLevelsPathExists = File.Exists destPeaksLevelsPath
                            let destPitchPathExists = File.Exists destPitchPath

                            let! sourcePath = async {
                                let! slices = async {
                                    ...
                                    if warm = "" then
                                        ...
                                        let openUnmixSeparationAsync i path = async {
                                            let modelId = sprintf "insanebrothers_%s_warm%s_v1" layer warm
                                            let modelPath = Path.Combine ((SharedConfig.pathsLazyIo ()).extOpenUnmix, "states", modelId)
                                            let inferenceScript = Path.Combine ((SharedConfig.pathsLazyIo ()).extOpenUnmix, "test.py")

                                            let! unmixedPath = async {
                                                let cmd = sprintf """python "%s" --model "%s" --outdir "%s" "%s" --targets %s"""
                                                              inferenceScript modelPath tempOutDir path layer

                                                let! errorCode, _ = Runtime.executeCondaAsync "mechahaze" [ cmd ]

                                                if errorCode <> 0 then
                                                    failwith "Error executing open unmix"

                                                return Path.Combine (tempOutDir, sprintf "%d.wav" i)
                                            }
                                ...
                                if not destPitchPathExists then
                                    let extractPitchAsync path = async {
                                        let cmd = sprintf """crepe "%s" --step-size 5 --model-capacity full --output "%s" """ path tempOutDir
                                        ...
                                        return Path.Combine (tempOutDir, (Path.GetFileName path).Replace (".wav", ".f0.csv"))
                                    }
    ...
    let listenTracksHealthAsync = async {
        Log.Debug ("TrackIngest tracks health listening start")

        while true do
            do!
                Directory.GetDirectories (SharedConfig.pathsLazyIo ()).dbTracks
