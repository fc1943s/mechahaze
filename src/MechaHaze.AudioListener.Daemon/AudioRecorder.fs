...
module SilentSinePlayer =
    let private hangIoAsync = async {
        ...
        with ex ->
            Log.Error (ex, "Error running SilentSinePlayer")
    }
...
module AudioRecorder = 
    let private unsafeRecordIoAsync =
        fun (succ, _, _) ->
            let onError ex = 
                exn ("Error recording audio", ex) |> Error |> succ
