...
module Main =
    let startAsync = async {
        ...
        let rabbitExchange =
            let rabbitBus = RabbitQueue.createBus configToml.RabbitAddress "root" "root"
        ...
        let init () =
            let state =
                ...
                | Error ex -> 
                    Log.Error (ex, "Error loading state. Default: {A}.", JsonConvert.SerializeObject SharedState.SharedState.Default)
                    raise ex
        ...
        let update (message: LocalQueue.Event) (state: SharedState.SharedState) : SharedState.SharedState * Cmd<LocalQueue.Event> =
            let newState, newCmd =
                match message with
                | LocalQueue.Error ex ->
                    ...
                    let newState =
                        ...
                        | Some stableTrack ->
                            ...
                            if unstableTrack.Id <> stableTrack.Id then 
                                ...
                                Log.Debug ("Track locking: {A}. Id: {B}. Offset: {C}", locked, stableTrack.Id, offset)
                ...
                | LocalQueue.ClientSetState newState ->
                    // TODO: this if...
                    if newState.BindingsPresetMap <> state.BindingsPresetMap then
    //                    Log.Debug ("Setting new time sync offset: {A}", newState.TimeSync)
                        rabbitExchange.Post "" (SharedState.StateUpdate newState)
                
                    newState, Cmd.none
                    
            let cleanState, cleanNewState =
                (state, newState)
                |> Tuple2.map SharedState.clean
                
            if cleanState <> cleanNewState then
                Log.Debug ("Persisting new state")
                
                match StatePersistence.writeIo cleanNewState with
                | Error ex -> Log.Error (ex, "Error persisting state. Default: {A}", JsonConvert.SerializeObject SharedState.SharedState.Default)
                | Ok () -> ()
        ...
        let subscription state =
            [ timerSub; eventInjectorSub ]
            |> Seq.map (fun x -> x state |> Cmd.ofSub)
            |> Cmd.batch
