...
module TrackMatcher =
    let private saveTempSampleIoAsync (sample: LocalQueue.Sample) = async {
        let outputFilePath = Path.Combine ((SharedConfig.pathsLazyIo ()).tempSamples, string (System.Random().Next ()) + ".wav")
        
        use writer = new WaveFileWriter (outputFilePath, Audio.NAudio.waveFormat)
        do! writer.WriteAsync (sample.Buffer, 0, sample.Buffer.Length) |> Async.AwaitTask
    ...
    let private queryFingerprintIoAsync (path: string) = async {
        try
            return!
                QueryCommandBuilder.Instance
                    ...
                    .Query (DateTime.UtcNow) |> Async.AwaitTask
        with ex ->
    ...
    let querySampleIoAsync (sample: LocalQueue.Sample) = async {
        ...
        if queryResult.ContainsMatches then
            ...
            if elapsed < 10. then
                ...
                Log.Debug (sprintf "Match Found. Id: {Id}. Position: {Position}. Elapsed: {Elapsed}", id, position, elapsed)
    ...
    module Stabilizer =
        ...
        let stabilizeTrack (oldTrack: SharedState.Track) (newTrack: SharedState.Track) : IoState<Io.State, SharedState.Track option> = Io.state {
            ...
            if oldTrack.Id = newTrack.Id then
                ...
                let (|Valid|Invalid|) = function
                    ...
                    | { SharedState.Position = oldPosition }, { Position = newPosition } when oldPosition = newPosition -> Invalid
                    | _ -> Valid
                ...
                if lastMatchesIds |> List.forall ((<>) oldTrack.Id) 
                   || lastMatchesIds |> List.forall ((=) newTrack.Id)
                then
                    state.Clear ()
