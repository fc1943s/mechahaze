...
module Waveform =
    ...
    type PitchSchema =
        CsvProvider<Sample=PitchSchemaSample, Schema="Time (decimal), Frequency (decimal), Confidence (decimal)", HasHeaders=true>
    ...
    let readPeaks fileName =
        ...
        let header =
            (reader.ReadUInt32 (), reader.ReadInt32 (), reader.ReadInt32 (), reader.ReadUInt32 (), reader.ReadInt32 ())
        ...
        let channels =
            seq {
                for _ in 0u .. header.Length - 1u do
            ...
            |> Seq.groupBy fst
            |> Map.ofSeq
            |> Map.map (fun _ v -> v |> Seq.map snd |> Seq.toArray)
            |> normalize
    ...
    let pitchCsvToWaveform (csvPath: string) peaksReferencePath destinationPath =
        let pitchRows =
            ...
            let normalized = seq { 0, pitchRows } |> Map.ofSeq |> normalize
        ...
        let peaks =
            ...
            |> Seq.map (fun i ->
                let peaksPitchIndex =
                    float i
                    * (float pitchRows.Length
                       / float reference.ChannelL.Length)
                    |> int
    ...
    let recombineCsvAsync slices destPath =
        async {
            let! slices =
                slices
            ...
            let slices =
                ...
                |> Array.mapi (fun i rows ->
                    if i = 0 then
                        ...
                        rows
                        |> Array.map (fun row ->
                            PitchSchema.Row (row.Time + previousRowTime, row.Frequency, row.Confidence)))
                |> Array.concat
